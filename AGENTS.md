...
- `src/proxy_optimizer.rs`: OpenAI proxy long-context compression, older-history summarization, and local summary cache handling.
...
- Keep feature ownership clear:
  ...
  - OpenAI proxy transport and Anthropic/OpenAI conversion belong in `src/proxy.rs`; context optimization, long-text compression, and summary caching belong in `src/proxy_optimizer.rs`.
