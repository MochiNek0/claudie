...
- `src/proxy/mod.rs`: proxy server, per-window profile routing by Bearer token (`resolve_openai_profile`/`match_profile_by_route_token`, falling back to the active profile), request optimization, upstream call/retry flow, upstream `Retry-After` forwarding, and `/v1/messages` routing.
...
- `src/settings/mod.rs`: settings/profile structs, normalization, OpenAI profile detection, extra env/body parsing, Claude settings writes, per-profile route token (`proxy_route_token`), `--settings` overlay generation (`~/.claudie/plans/<id>.json`) and launch-command string, and onboarding helper.
...
- `%USERPROFILE%\.claudie\plans\<id>.json`: per-profile `--settings` overlay files written by "copy launch command" so a specific Claude Code window binds to one profile.
