...
use capability_cache::{
    cached_tool_history_known_native, cached_tool_history_needs_transcript,
    prefill_tool_history_capability, save_tool_history_capability,
};
use http::{read_http_request, write_json_response};
...
fn handle_proxy_client(mut stream: TcpStream, state: Arc<Mutex<AppState>>, agent: ureq::Agent) {
    ...
    prefill_tool_history_capability(&profile, &openai_request);
