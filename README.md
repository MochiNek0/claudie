...
- 支持番茄钟、空闲睡眠状态、宠物缩放和 mood 到 GIF 的映射配置。
- 右键菜单提供 Settings、Start/Stop/Pause/Resume/Skip Pomodoro 和 Exit。
- Settings 面板包含 Basic、Pomodoro、LLM Profiles 三个标签页。
- 支持保存 LLM provider/profile，并把当前 profile 写入 Claude Code settings。
...
代理当前实现了 Claude Code 常用的 `POST /v1/messages`、`POST /v1/messages/count_tokens` 和 `GET /v1/models`，并支持把工具调用在 Anthropic/OpenAI 格式之间转换。
...
src/
  ...
  ui/
    ...
    window/                主宠物窗口
      mod.rs               窗口生命周期、热键、菜单、点击处理
      render.rs            HUD、宠物、权限按钮和选择卡片绘制
...
- `%USERPROFILE%\.claudie\settings.json`：宠物资源路径、GIF 目录、动画映射、缩放、睡眠时间和番茄钟设置。
- `%USERPROFILE%\.claudie\llm_profiles.json`：LLM provider/profile 定义。
...
重点检查宠物窗口、右键菜单、Basic/Pomodoro/LLM Profiles tabs、GIF 资源加载、`POST /hook` 状态更新、权限/选择卡片，以及需要时的本地 LLM 代理。
