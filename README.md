...
- **多会话切换** — 跟踪各会话状态，在宠物旁渲染会话切换面板（每行显示该会话所用模型 id），滚轮切换关注的会话；焦点会话决定宠物 mood 与 HUD。仅单会话时自动隐藏。按 ESC 中断本轮后宠物会回到空闲；已关闭的会话（包括不发 `SessionEnd` 的强制退出）会自动移除。
- **多窗口分流** — 代理按 Bearer Token 路由到对应 profile，多个 Claude Code 窗口可共享同一本地代理却各自连不同上游。「复制启动命令」会写出一份 `~/.claudie/plans/<id>.json` 覆盖文件，并把 `claude --settings "<path>"` 拷到剪贴板，从设置面板按钮或托盘子菜单触发。
...
代理按请求里的 Bearer Token 路由到对应 profile（profile 的 `auth_token`，留空时派生为 `claudie-<id>`），未命中则回退到当前激活 profile。想同时开多个分别走不同上游的 Claude Code 窗口时，用「复制启动命令」生成各 profile 的 `claude --settings "<path>"` 即可。
...
| `plans/` | 每个 profile 的 `--settings` 覆盖文件（`<id>.json`），供「复制启动命令」绑定某个窗口 |
