...
- Supports a Pomodoro timer, idle sleep state, pet scaling, and mood-to-GIF mapping.
- Right-click menu includes Settings, Start/Stop/Pause/Resume/Skip Pomodoro, and Exit.
- Settings panel includes Basic, Pomodoro, and LLM Profiles tabs.
- Saves LLM providers/profiles and writes the active profile into Claude Code settings.
...
The proxy currently implements the Claude Code endpoints `POST /v1/messages`, `POST /v1/messages/count_tokens`, and `GET /v1/models`, and it converts tool calls between Anthropic and OpenAI formats.
...
src/
  ...
  ui/
    ...
    window/                Main pet window
      mod.rs               Window lifecycle, hotkeys, menu, click handling
      render.rs            HUD, pet, permission button, and choice card drawing
...
- `%USERPROFILE%\.claudie\settings.json`: pet asset path, GIF directory, animation mapping, scale, sleep timeout, and Pomodoro settings.
- `%USERPROFILE%\.claudie\llm_profiles.json`: LLM provider/profile definitions.
...
Check the pet window, right-click menu, Basic/Pomodoro/LLM Profiles tabs, GIF asset loading, `POST /hook` state updates, permission/choice cards, and the local LLM proxy when relevant.
